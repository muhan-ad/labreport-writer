...
def _generate_docx(data: dict, output_path: str):
    ...
    doc.add_paragraph("式中 M 为互感系数，I₀ 为初级电流，d̄m = (dm₁ + dm₂) / 2 为两次偏转平均值。")
    ...
    rkb_rows.append(["均值", "", "", "", "", f"{RKb_mean:.3e}"])
    ...
    doc.add_math(
        rf"RK_b = \frac{{{M_H:.3e} \times {i0_ma_list[0]/1000:.3f}}}"
        rf"{{{dm_bar_mm_list[0]:.1f}}}"
        rf" = {rkb_list[0]:.3e} \,\mathrm{{C\cdot\Omega/mm}}"
    )

    doc.add_paragraph("三组 RKb 的平均值：")
    doc.add_math(
        rf"\overline{{RK_b}} = {RKb_mean:.3e} \,\mathrm{{C\cdot\Omega/mm}}"
    )

    # RKb 不确定度
    doc.add_paragraph("RKb 的不确定度评定：")
    doc.add_paragraph("A 类不确定度（3 次测量）：")
    doc.add_math(rf"u_{{\mathrm{{A}}}}(RK_b) = {u_A_RKb:.3e} \,\mathrm{{C\cdot\Omega/mm}}")
    doc.add_paragraph("B 类不确定度（标尺 Δ仪 = 0.5 mm + 电流表 Δ仪 = 1.0 mA）：")
    doc.add_math(rf"u_{{\mathrm{{B}}}}(RK_b) = {u_B_RKb:.3e} \,\mathrm{{C\cdot\Omega/mm}}")
    doc.add_paragraph("合成不确定度：")
    doc.add_math(rf"u(RK_b) = \sqrt{{u_{{\mathrm{{A}}}}^2 + u_{{\mathrm{{B}}}}^2}} = {u_RKb:.3e} \,\mathrm{{C\cdot\Omega/mm}}")
    ...
    doc.add_math(
        ...
        rf" = {B0_theory:.3e} \,\mathrm{{T}}"
    )

    doc.add_paragraph("中心点实验值：")
    doc.add_math(
        rf"B = \frac{{\overline{{RK_b}}}}{{nS}} d_m"
        rf" = \frac{{{RKb_mean:.3e}}}{{{n_coil:.0f} \times {S_m2:.2e}}}"
        rf" \times {dm_mm_center:.1f}"
        rf" = {B_exp_center:.3e} \,\mathrm{{T}}"
    )

    doc.add_paragraph("相对误差：")
    doc.add_math(
        rf"E = \left|\frac{{B_0 - B}}{{B_0}}\right| \times 100\%"
        rf" = \left|\frac{{{B0_theory:.3e} - {B_exp_center:.3e}}}"
        rf"{{{B0_theory:.3e}}}\right| \times 100\%"
        rf" = {E_rel:.2f}\%"
    )
    ...
    cmp_rows = [
        ["B (x=0)", f"{B0_theory:.3e}", f"{B_exp_center:.3e}", f"{E_rel:.1f}%"],
    ]
...
def main():
    ...
    DOCX_FILE = os.path.join(SCRIPT_DIR, "用冲击法测量螺旋管磁场分布实验.docx")
