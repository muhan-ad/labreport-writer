...
本项目由 **慕寒** 与 **宇宙创生** 联合开发完成。项目核心架构设计、用户界面实现及整体技术方案规划由慕寒负责开发与调试；宇宙创生提供了西安电子科技大学物理实验课程知识库、基于 Word COM 接口的报告自动生成脚本，以及对策府库的实验数据集。本项目不提供实验数据生成，仅提供模板参考，一切实验报告均需用户亲自检查，校验，成果仅供参考，如有问题概不负责！

本项目的实验数据资源得益于对策府库开源社区的贡献，在此向对策府库全体贡献者致以诚挚的谢意，感谢其在实验数据共享与开源精神方面所做出的努力。

如对本项目有任何疑问、建议或合作意向，欢迎通过以下方式联系：对策府库官方群聊。
...
- **章节级润色与导入重生成**：为降低同质化，本项目特设计同一实验由不同变体组合，变体包括实验原理/实验方法/误差分析/结论，只有变体部分可以使用润色。变体润色以生成时缓存的**章节源文**为原文（公式可保持 LaTeX 原码），润色结果可一键**导入并重新生成报告**——Markdown 与 LaTeX 公式经归一化管线自动转换：`$...$`/`$$...$$` 渲染为 Word 原生公式，标题/加粗/列表等 Markdown 记号自动剥离，AI 输出中不规范的裸 LaTeX 与跨行公式块亦有自动修复兜底。数据表格、插图与数值计算不受润色影响；（也即是说，只有变体类型的部分支持ai润色，仅复制类型只能手动修改）
...
设置 → **检查更新**：内置更新清单地址（国内对象存储 COS 直链，`latest.json` 含版本号/更新说明/安装包直链）。点击「检查更新」：已是最新版本会提示；发现新版本后**自动下载**并在完成后打开安装程序，下载进度实时显示，可随时取消。发布流程见 [RELEASE.md](RELEASE.md)。
...
| 事项 | 状态 | 说明 |
| --- | --- | --- |
| **7 个实验暂无变体库** | 开发中 | 劈尖干涉、霍尔效应实验、衍射光强分布的测量、理想气体状态方程、直螺线管磁场分布的测量、电子元件伏安特性的测量、电子偏转特性的测量——这 7 个实验可正常生成报告，但章节文字为固定表述，**不支持 AI 润色"导入并重新生成"**（仅可复制润色，结果手动使用），变体选择面板亦为空。补齐 `variants.json` 后即与其余 19 个实验能力对齐（变体的生成会陆续补全） |
